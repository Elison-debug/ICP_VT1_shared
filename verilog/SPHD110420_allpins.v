...
/****************************************************************
--  Description         : verilog_allpins_model for ST_SPHDL cmos65
--  ModelVersion        : 4.4
--  Date                : Jun, 2008
--  Changes Made by	: DRM
--
...
module ST_SPHDL_160x32m8_L (Q, RY,CK, CSN, TBYPASS, WEN, A, D    , gnd, vdd  ); 
    parameter
  ...
  assign is_supply_ok = (vddint === 1'b1 && gndint === 1'b0  ) ? 1'b1 : 1'b0;
	...
	wire WENint;
        ...
        wire RYint;
           ...
           assign RY = (is_supply_ok === 1'b1) ? RYint : 1'bx;   
	...
	reg ValidDebugCode;
        ...
        integer flag_error;
          ...
          assign Q = (is_supply_ok === 1'b1) ? Qint : WordX;   
	...
	buf (CKint, CK);
        ...
        or (CSNint, CSN, TBYPASS, !is_supply_ok);   
...
always @(vddint or gndint )
begin
   if($realtime != 0) begin
   if(debug_level < 2) $display("%m - %t (MSG_ID 018) WARNING: Illegal Value on power supply pin(s). Memory and Output Corrupted ",$realtime);
   WriteMemX;
   WriteOutX;
   delOutReg_data= WordX;
   Qint = WordX;
...
module ST_SPHDL_160x32m8_L_main ( Q_glitch,  Q_data, Q_gCK, RY_rfCK, RY_rrCK, RY_frCK, ICRY, delTBYPASS, TBYPASS_D_Q, TBYPASS_main, CK,  CSN, TBYPASS, WEN,  A, D, M,debug_level ,TimingViol0, TimingViol1, TimingViol2, TimingViol3, TimingViol4, TimingViol5, TimingViol6, TimingViol7, TimingViol8, TimingViol9, TimingViol10, TimingViol_ttms_ttmh, TimingViol12, TimingViol13    , is_supply_ok, vdd, gnd );
  ...
  $display ("***************************************");
   ...
   buf bufAint [Addr-1:0] (Aint, A);
	...
	reg TimingViol9_tck_ry, TimingViol10_taa_ry;
       ...
       assign RY = (is_supply_ok === 1'b1) ? RY_outreg : 1'bx;     
        always @(RYint)
        begin
        RY_outreg = RYint;
        end
...
module ST_SPHDL_1216x32m16_bL (Q, RY,CK, CSN, TBYPASS, WEN, A, D , M   , gnd, vdd  ); 
    parameter
...
module ST_SPHDL_160x64m8_L (Q, RY,CK, CSN, TBYPASS, WEN, A, D    , gnd, vdd  ); 
    parameter
...
module ST_SPHDL_640x32m16_L (Q, RY,CK, CSN, TBYPASS, WEN, A, D    , gnd, vdd  ); 
    parameter
...
module ST_SPHDL_160x64m8_bL (Q, RY,CK, CSN, TBYPASS, WEN, A, D , M   , gnd, vdd  ); 
    parameter
